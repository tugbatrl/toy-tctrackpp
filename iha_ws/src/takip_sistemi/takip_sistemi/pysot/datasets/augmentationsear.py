...
class Augmentations:
    ...
    def _shift_scale_aug(self, image, bbox, crop_bbox, size,rand):
        im_h, im_w = image.shape[:2]

        # adjust crop bounding box
        crop_bbox_center = corner2center(crop_bbox)
        if self.scale:
            scale_x = (1.0 + rand[0] * self.scale)
            scale_y = (1.0 + rand[1] * self.scale)
        ...
        if self.shift:
            sx = rand[2] * self.shift
            sy = rand[3] * self.shift
    ...
    def __call__(self, image, bbox, size, rand, gray=False):
        shape = image.shape
        ...
        image, bbox = self._shift_scale_aug(image, bbox, crop_bbox, size,rand)
